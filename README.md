...
| §3 Onboarding wizard | 🟡 (a)+(b)+(c) shipped 2026-04-29 — vault + schema + IPC surface + React wizard/unlock screen; (d) pending |
...
- **Encrypted at rest** — per-domain encryption with operator-derived master key (Phase 1 §2, closed 2026-04-29). Argon2id KDF (m=65536, t=3, p=1) + HKDF-SHA256 per-domain derivation + XChaCha20-Poly1305 AEAD envelope (24-byte nonce, 6-byte header bound as AAD). §3 (a) wired the vault layer on 2026-04-29: passphrase sentinel at `~/.coo/sentinel` + `vault::setup_passphrase`/`vault::unlock` state machine, with `crypto::derive_lock_key` (HKDF info `coo/v1/lock`) sibling to the four state-domain derivations. §3 (b) added migration #2 on 2026-04-29: `app_config` (plaintext key/value), `operator_profile` (singleton-via-CHECK + encrypted BLOB), `calibration_setting` (placeholder key/value with encrypted BLOB; typed schema deferred to §6). Encrypted-column convention established: plaintext identity columns + a single ciphertext BLOB carrying the §2 (c) v1 bundle. §3 (c) shipped on 2026-04-29: c1 added the Tauri IPC surface (managed `AppState` with vault + db, six commands wrapping inspect/setup/unlock and the three encrypted writes); c2 added the React wizard (welcome → passphrase → profile → review with Familiar preset hardcoded) plus the unlock screen and error/placeholder routing in `App.tsx`. End-to-end UI flow ready for operator verification via `npm run tauri dev`.
